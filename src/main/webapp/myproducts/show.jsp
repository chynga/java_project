<%@ page import="com.egadgets.demo3.model.Product" %>
<%@ page import="com.egadgets.demo3.model.User" %>
<%@ page import="com.egadgets.demo3.model.Message" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="com.egadgets.demo3.model.Writer" %>
<jsp:include page="../header.jsp"/>

<div class="container">
    <div class="row">
        <%
            Product product = (Product) request.getAttribute("product");
            User seller = (User) request.getAttribute("seller");
            ArrayList<Message> chat = (ArrayList<Message>) request.getAttribute("chat");
            ArrayList<User> customers = (ArrayList<User>) request.getAttribute("customers");
            int customerId = (int) request.getAttribute("customer_id");
        %>

        <div class="card col-sm-6" style="width: 18rem;">
            <%
                if (product != null) {
                    String productImageURL = null;
                    try {
                        productImageURL = product.getImage().getImageUrl();
                    } catch (Exception e) {
                        productImageURL = "data:image/jpeg;base64,";
                    }

                    out.print("<img src=" + productImageURL + " class=\"card-img-top\">");
                    out.print("<div class=\"card-body\">");
                    out.print("<h5 class=\"card-title\">" + product.getProductName() + "</h5>");
                    out.print("<p class=\"card-text\">" + product.getDescription() + "</p>");
                    out.print("</div>");
                    out.print("<ul class=\"list-group list-group-flush\">");
                    out.print("<li class=\"list-group-item\">Price: " + product.getPrice() + "$</li>");
                    out.print("</ul>");

                    out.print("<div class=\"card-body\">");
                    out.print("<form action='" + request.getContextPath() + "/myproducts/show' method='POST'>");
                    out.print("<input type=\"hidden\" name=\"id\" value=" + product.getId() + ">");
                    out.print("<input type=\"hidden\" name=\"action\" value='delete'>");
                    out.print("<button type=\"submit\" class=\"btn btn-danger\">Remove Product</button>");
                    out.print("</form>");
                    out.print("</div>");
                } else {
                    out.print("ERRrrror");
                }
            %>
        </div>

        <div class="card col-sm-6" style="width: 18rem;">
            <form action="">
                <h2>Write to:</h2>
                <div class="dropdown" display="form-inline">
                    <select class="custom-select" aria-label="Default select example" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                        <option selected>Select Customer</option>
                        <%
                            for (User customer : customers) {
                                out.print("<option value='" + request.getContextPath() + "/myproducts/show?id=" + product.getId() + "&customer_id=" + customer.getId() + "'>" + customer.getUsername() + "</option>");
                            }
                        %>
                    </select>
                </div>
            </form>

<%--            <form action="${pageContext.request.contextPath}/products/show" method="POST" class="mb-3 validated-form" novalidate>--%>
<%--                <input type="hidden" name="id" value="${product.getId()}">--%>
<%--                <div class="mb-3">--%>
<%--                    <label class="form-label" for="body">Write Your Text</label>--%>
<%--                    <textarea class="form-control" id="body" name="body" cols="30" rows="3" required></textarea>--%>
<%--                    <div class="valid-feedback">--%>
<%--                        Looks good!--%>
<%--                    </div>--%>
<%--                </div>--%>
<%--                <button class="btn btn-success">Send</button>--%>
<%--            </form>--%>

            <form action="${pageContext.request.contextPath}/myproducts/show" method="POST" class="mb-3 validated-form" novalidate>
                <input type="hidden" name="id" value="${product.getId()}">
                <input type="hidden" name="customer_id" value="<%out.print(customerId);%>">
                <input type="hidden" name="action" value="write">
                <div class="mb-3">
                    <label class="form-label" for="body">Write Your Text</label>
                    <textarea class="form-control" id="body" name="body" cols="30" rows="3" required></textarea>
                    <div class="valid-feedback">
                        Looks good!
                    </div>
                </div>
                <button class="btn btn-success">Submit</button>
            </form>

            <div class="card mb-3 ">
                <div class="card-body">
                    <%
                        if (chat != null) {
                            for (Message message : chat) {
                                if (message.getWriter() == Writer.SELLER) {
                                    out.print("<h6 class=\"card-subtitle text-muted text-right\">Me</h6>");
                                    out.print("<p class=\"card-text text-right\">" + message.getSentence() + "</p>");
                                } else if (message.getWriter() == Writer.CUSTOMER) {
                                    out.print("<h6 class=\"card-subtitle text-muted\">Customer</h6>");
                                    out.print("<p class=\"card-text\">" + message.getSentence() + "</p>");
                                }
                            }
                        } else {
                            out.print("ERRrrror");
                        }
                    %>
                </div>
            </div>
        </div>
    </div>
</div>
<script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>

<jsp:include page="../footer.jsp"/>